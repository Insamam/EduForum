...
const QuestionDetail = () => {
  ...
  const handleVerifyAnswer = async (answerId) => {
    if (!isTeacher) {
      alert("Only teachers can verify answers.");
      return;
    }
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 bg-white rounded-2xl shadow-xl transition-all duration-300 ease-in-out">
      ...
      <div>
        ...
        {answers.map((answer) => (
          <div key={answer.id} className={`p-6 rounded-xl mb-6 border transition-shadow duration-300 hover:shadow-md ${
            answer.is_verified ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'
          }`}>
                  ...
                  <div className="flex-1">
                    ...
                    {answer.is_verified && (
                      <div className="mt-2 flex items-center text-green-600">
                        ...
                        <span className="text-sm font-medium">Verified by Teacher</span>
                      </div>
                    )}
                  </div>
                  <div className="flex gap-2">

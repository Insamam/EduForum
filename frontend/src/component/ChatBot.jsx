    ...
    function ChatBot() {
        ...
        return (
            <div className="flex flex-col h-screen w-screen bg-gray-10">
                {/* Chat Messages Container with Equal Padding */}
                <div ref={chatContainerRef} className="flex-1 overflow-y-auto px-80 py-4"> {/* Added px-16 for equal horizontal padding */}
                    {messages.map((message, index) => (
                        <div
                            key={index}
                            className={`max-w-[50%] px-4 py-2 rounded-lg ${
                                message.role === 'user'
                                 ? 'bg-gray-300 text-blue self-end ml-auto text-right w-full' 
                ...
                {/* Input Field and Send Button with Equal Padding */}
                <div className="flex items-center gap-2 px-4 py-3 bg-white border-t border-gray-30 w-full"> {/* Added px-16 for equal horizontal padding */}
                    <input
                        type="text"
                        value={input}
